...
async def send_telegram_message(chat_id: str, message: str, reply_to_message_id: Optional[str] = None) -> Dict:
...
async def send_interactive_buttons(
    chat_id: str, message: str, buttons: List[Dict[str, str]], reply_to_message_id: Optional[str] = None
) -> Dict:
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(url) as response:
                if response.status >= 400:
                    error_text = await response.text()
                    logger.error(f"Telegram webhook deletion error: {error_text}")
                    raise HTTPException(
                        status_code=response.status,
